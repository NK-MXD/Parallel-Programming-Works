...
/*void matrix_multiply_matrix_sse_title(int n, float**a, float**b,float**c){
    //__m128 t1,t2,sum;
    //float t;
    /*long long head,tail,freq;
    QueryPerformanceFrequency((LARGE_INTEGER*)&freq);
...
double coo_multiply_matrix_avx(int nonzeros, int n, int* row, int* col, float* value, float** b, float** c) {
    ...
    for (int i = 0; i < nonzeros; i++)
    {
        for (int k = 0; k < n - choice; k += 8)
        {
            ...
            t1 = _mm256_load_ps(b[col[i]] + k);
            t2 = _mm256_load_ps(c[row[i]] + k);
            t3 = _mm256_set1_ps(value[i]);
            sum = _mm256_mul_ps(t3, t1);
            t2 = _mm256_add_ps(t2, sum);
            _mm256_store_ps(c[row[i]] + k, t2);
        }
        for (int k = n - choice; k < n; k++) {
            c[row[i]][k] += value[i] * b[col[i]][k];
        }
    ...
    QueryPerformanceCounter((LARGE_INTEGER*)&tail);
    cout<<"avx:"<<(double)(tail-head)/(double)freq<<endl;
    return (double)(tail-head)/(double)freq;
}
...
int main()
{
    ...
    cout<<"平均耗时:"<<endl;
    cout<<"串行算法耗时:"<<serial_mul_sum/10<<"s"<<endl;
    cout<<"SSE1优化耗时:"<<sse1_mul_sum/10<<"s"<<endl;
    cout<<"SSE2优化耗时:"<<sse2_mul_sum/10<<"s"<<endl;*/
    ///下面进行不同规模数据的采集与收集
    /*for(int j=1;j<10;j++){
        int n=j*1024;
        srand((int)time(0));
