/**
-----------------------------------------------------------------------
本实验是openMP来优化稀疏矩阵相乘的算法，openMP的静态分配策略，动态分配策略,
另外可以思考如何实现串行算法的第二种并行方法

注意：本实验的目标是实现coo格式的静态动态等线程分配，另外思考其他线程分配的粒度和方式
注意：本实验还要实现在HYB格式的稀疏矩阵相乘并行化
------------------------------------------------------------------------
windows版本
codeblock
**/
...
//实现COO和向量相乘（串行算法）
double coo_multiply_vector_serial(){
    //其中x指的是列向量，这里表示的是稀疏矩阵和列向量相乘
    long long head,tail,freq;
    QueryPerformanceFrequency((LARGE_INTEGER*)&freq);
    QueryPerformanceCounter((LARGE_INTEGER*)&head);
    for (int i=0;i<nonzeros;i++)
        y[row[i]] += value[i] * vec[col[i]];//最后y中得出的结果是最后的稀疏矩阵和列向量相乘的结果
    /*for(int i=0;i<nozerorows;i++)
    {
        for(int j=index[i];j<index[i+1];j++)
        {
            y[row[j]]+=value[j]*vec[col[j]];
        }
    }*/
...
//基本思路是循环展开
double coo_multiply_matrix_sse(){
    long long head,tail,freq;
    QueryPerformanceFrequency((LARGE_INTEGER*)&freq);
    QueryPerformanceCounter((LARGE_INTEGER*)&head);
    __m128 t1,t2,t3,sum;
...
//实现原理是实现八路并行
double coo_multiply_matrix_avx(){
    long long head,tail,freq;
    QueryPerformanceFrequency((LARGE_INTEGER*)&freq);
    QueryPerformanceCounter((LARGE_INTEGER*)&head);
    __m256 t1, t2, t3, sum;
...
//pThread实现spMM-SSE代码封装,这是静态的sse结合的代码
...
//pThread实现spMM-AVX代码封装，这是静态的avx八路并行代码
...
void spMM_all_test(){
    ...
    spmm_sse=coo_multiply_matrix_sse();
    spmm_avx=coo_multiply_matrix_avx();
...
///--------------------------------------------------openMP算法的一些测试-------------------------------------------
void spMV_openMP_test(){
    init();
    cout<<coo_multiply_vector_openMP_static()
        <<endl<<coo_multiply_vector_openMP_dynamic()
        <<endl<<coo_multiply_vector_serial()
        <<endl<<spMV_pThread_static(4);
}
...
void spMM_openMP_test(){
    ...
    cout<<coo_multiply_matrix_serial()
        ...
        <<endl<<spMM_pThread_static1(4)
        //<<endl<<coo_multiply_vector_openMP_dynamic()
        //<<endl<<spMM_pThread_static1(4);
        <<endl<<spMM_pThread_dynamic1(4)
        <<endl<<coo_multiply_matrix_openMP_static_sse()
        <<endl<<coo_multiply_matrix_openMP_dynamic_sse();
}
...
int main(int argc, char* argv[])
{
    init();
    spMM_all_test();
    cout<<coo_multiply_matrix_mpi_dynamic_avx(argc,argv)<<endl
        <<coo_multiply_matrix_serial();
