/**
-----------------------------------------------------------------------
本次实验是实现MPI并行的稀疏矩阵COO格式的乘法的运算,写一些优化思路要求：

1. MPI加进来，运行成功；
2. 重新整理程序，使得程序执行更加有条理，参数也调节更加方便；
3. 修改MPI中的进程数目，探索设置不同进程的执行时间的差别；
...
int OMP_NUM_THREADS=4;//openMP需要用到的线程的数目
...
double coo_multiply_vector_mpi(int argc, char* argv[]){
    ...
    {
        for(int j=myindex[i];j<myindex[i+1];j++)
        {
            yy[row[j]]+=value[j]*vec[col[j]];
        }
    }
...
///实现MPI+openMP+static+neon
double coo_multiply_matrix_mpi_static_neon(int argc, char* argv[]){
    struct timeval val,newVal;
    gettimeofday(&val, NULL);
    int myid, numprocs;
    MPI_Init(&argc,&argv);
...
///实现MPI+openMP+dynamic+neon
double coo_multiply_matrix_mpi_dynamic_neon(int argc, char* argv[]){
    struct timeval val,newVal;
    gettimeofday(&val, NULL);
    int myid, numprocs;
    MPI_Init(&argc,&argv);
...
void spMM_all_test(){
    ...
    spMM_static_neon=spMM_pThread_static_neon(4);
    next_arr2 = 0;
...
int main(int argc, char* argv[])
{
    ...
    //spMM_all_test();
    double spmm_mpi;
    spmm_mpi=coo_multiply_matrix_mpi_dynamic_neon(argc,argv);
    cout<<"spmm_mpi_dynamic_neon,"<<spmm_mpi<<endl;
    //释放内存空间
    delete []mat;
